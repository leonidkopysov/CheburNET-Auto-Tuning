...
![Version](https://img.shields.io/badge/version-1.0.1-8b5cf6?style=for-the-badge)
...
**ЧебурNET Auto Tuning** — скрипт настройки и обслуживания Linux-серверов,
на которых работают Xray и ноды Remnawave. Он помогает подготовить сервер
к сетевой нагрузке, настроить системные лимиты и проверить базовую защиту.

Скрипт подбирает параметры сети по количеству процессорных ядер и объёму
оперативной памяти, настраивает BBR и ZRAM, проверяет лимиты ноды, SSH,
firewall и доступ к API. Дополнительно проверяет обновления безопасности,
синхронизацию времени, состояние диска и автопродление сертификатов.

Подходит для первоначальной настройки и повторной диагностики сервера.
Перед изменениями сохраняет снимки настроек, а в конце показывает результат
проверок и предупреждения. Сам Xray и панель Remnawave не устанавливает.
